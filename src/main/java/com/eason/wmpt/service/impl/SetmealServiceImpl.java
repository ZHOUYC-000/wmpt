...
public class SetmealServiceImpl extends ServiceImpl<SetmealMapper, Setmeal> implements SetmealService {
    ...
    public SetmealDto getByIdWithDish(Long id) {
        ...
        List<SetmealDish> dishs = setmealDishService.list(lambdaQueryWrapper);
    ...
    @Override
    public void updateSatusWithDish(List<Long> ids, int theStatus) {
        List<Setmeal> setmeals = this.listByIds(ids);
        setmeals = setmeals.stream().map(item ->{
            item.setStatus(theStatus);
            return item;
        }).collect(Collectors.toList());
